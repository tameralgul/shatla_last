...
<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
    ...
    <div class="d-flex align-items-center flex-wrap mr-2">
        ...
        <span class="text-muted font-weight-bold mr-4">الصفحات</span>
        <!--end::Actions-->
    </div>
</div>
        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-3">
                <div class="card-title">
                    <h3 class="card-label">الصفحات
                        <span class="d-block text-muted pt-2 font-size-sm">عرض جميع &amp; الصفحات</span>
                    </h3>
                </div>
                @if (App\Models\Page::count() >= 5)
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="card-body table-responsive p-0">
                                <table class="table table-hover text-nowrap table-bordered" id="pages_datatable">
                                    <thead>
                                        <tr>
                                            <th width="3%">#</th>
                                            <th width="13%">عنوان الصفحة</th>
                                            <th width="13%">وصف الصفحة</th>
...
<!-- Modal -->
<div class="modal fade" id="edit_page_Modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body p-0">
                <form class="form" id="edit_page_form">
                    <div class="card-body">
                        <div class="form-group">
                            <label>العنوان :</label>
                            <div class="input-icon input-icon-right">
                                <input name="title" type="text" id="EditTitle" class="form-control" placeholder="" />
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <label>النص :</label>
                            <div class="input-icon input-icon-right">
                                <textarea name="description" type="text" id="EditDescription" class="form-control" placeholder="" ></textarea>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    oTable = $('#pages_datatable').DataTable({
    "paging": true,
    "lengthChange": true,
    "searching": true,
    "ordering": true,
...
<script>
    $(function () {
        ...
        $(document).on('click', '.editPage', function (event) {
        var page_id = $(this).data('id');
        // alert(category_id);
           $.ajax({
               url:'/dashboard/pages/edit/' + page_id,
               method:'get',
               success:function(data) {
                   $('#editmodelHeading').html("تعديل الصفحة");
            ...
            $(document).on('click','#EditsaveBtn',function() {
               let page_form = document.getElementById('edit_page_form');
               let form_data = new FormData(page_form);
               $.ajax({
                url:"/dashboard/pages/update",
                method:'post',
                data:form_data,
                dataType:'json',
                ...
                var oTable = $('#pages_datatable').dataTable();
                    oTable.fnDraw(false);
...
<script>
    ...
    $('#btn_show_modal').on('click',function(event) {
       ...
       $("#modelHeading").html("أضافة صفحة جديد");
        $("#page_form").trigger('reset');//to clear the form
...
<script>
    ...
    Swal.fire({
    title: 'هل أنت متأكد ؟',
    text: "هل أنت متأكد من حذف الصفحة " + page_name,
    icon: 'warning',
    ...
    $.ajax({
    type: "get",
    url: SITEURL + "/dashboard/pages/delete/" + deleteID,
    success: function (response) {
    if (response.status == 200) {
        toastr.success(response.success);
    }
